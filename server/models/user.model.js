...
const UserSchema = new mongoose.Schema(
  {
    ...
    password: {
      ...
      validate: {
        validator: (val) => /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[^\w\s]).{8,}$/.test(val),
        message: "Password rule: at least 8 characters, at least 1 numeric character, at least 1 lowercase letter, at least 1 uppercase letter and at least 1 special character",
